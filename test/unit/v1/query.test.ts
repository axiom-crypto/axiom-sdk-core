...
describe('Axiom Query', () => {
  ...
  test('should get a ResponseTree for a keccakQueryResponse', async () => {
    const responseTree = await (ax.query as QueryV1).getResponseTreeForKeccakQueryResponse("0x7dc6fdd0fd0526ff4e84771317c54b23e99d4ae940c37f3f89ff93ecc5f559b0");
    expect(responseTree?.data[0]?.rowHash).toEqual("0x125a2f44a96398e819f5b5da7cb0bf5fe8060eba423a106b1196719a1ac3340d");
  });
  ...
  test('should get a validation witness for a ResponseTree', async () => {
    ...
    const validationWitness = (ax.query as QueryV1).getValidationWitness(responseTree, blockNumber, address, slot);
  ...
  test('should get queryHash from txHash', async () => {
    ...
    const queryHash = await (ax.query as QueryV1).getQueryHashFromTxHash(txHash);
  ...
  test('should get keccakQueryResponse from txHash', async () => {
    const txHash = TX_HASH;
    const keccakQueryResponse = await (ax.query as QueryV1).getKeccakQueryResponseFromTxHash(txHash);
  ...
  test('should get the ResponseTree from a txHash', async () => {
    const txHash = TX_HASH;
    const responseTree = await (ax.query as QueryV1).getResponseTreeFromTxHash(txHash);
    expect(responseTree?.data[0]?.rowHash).toEqual("0x125a2f44a96398e819f5b5da7cb0bf5fe8060eba423a106b1196719a1ac3340d");
  });
  ...
  test('should get keccakBlockResponse', () => {
    ...
    const keccakBlockResponse = (ax.query as QueryV1).getKeccakBlockResponse(blockHash, blockNumber);
  ...
  test('should get keccakAccountResponse', () => {
    ...
    const keccakAccountResponse = (ax.query as QueryV1).getKeccakAccountResponse(blockNumber, address, nonce, balance, storageRoot, codeHash);
  ...
  test('should get keccakStorageResponse', () => {
    ...
    const keccakStorageResponse = (ax.query as QueryV1).getKeccakStorageResponse(blockNumber, address, slot, value);
